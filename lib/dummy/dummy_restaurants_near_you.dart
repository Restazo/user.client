import 'package:restazo_user_mobile/models/restaurant_near_you.dart';

final restaurants = [
  const RestaurantNearYou(
    id: 'df7f6d73-0580-402a-ae3e-97db9152fb18',
    addressLine: "Willow Walk 66",
    affordability: 2,
    coverImage:
        'https://images.unsplash.com/photo-1546964124-0cce460f38ef?q=80&w=2940&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    description:
        "A slice of heaven in every bite. Artisanal pizzas crafted with love.",
    name: "Curry Corner",
  ),
  const RestaurantNearYou(
    id: 'd0ad3d57-a941-4d16-89ee-04f5a621daf5',
    addressLine: "Pine Lane 33",
    affordability: 2,
    coverImage:
        'https://images.unsplash.com/photo-1615937657715-bc7b4b7962c1?q=80&w=2940&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    description:
        "Savor the spice of life with our authentic curries, made from ancient recipes.",
    name: "Gourmet Gala",
  ),
  const RestaurantNearYou(
    id: 'b819fe16-ab04-4b48-b744-e179bb545dc5',
    addressLine: "Willow Walk 66",
    affordability: 3,
    coverImage:
        'https://images.unsplash.com/photo-1611171711791-b34fa42e9fc2?q=80&w=3087&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    description:
        "Traditional tastes meet contemporary flair. A truly unique dining experience.",
    name: "Gourmet Gala",
  ),
  const RestaurantNearYou(
    id: 'd307e29e-5f77-4d26-a506-53b5e2db7ef2',
    addressLine: "Elm Road 101",
    affordability: 3,
    coverImage:
        'https://images.unsplash.com/photo-1571162242324-f1607b1eceaa?q=80&w=2940&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    description:
        "Sweet, savory, and sensational. A dessert-first dining experience.",
    name: "Burger Bonanza",
  ),
  const RestaurantNearYou(
    id: 'c71a9024-5b6c-4c7e-aabb-298f5e17cb62',
    addressLine: "Walnut Way 213",
    affordability: 1,
    coverImage:
        'https://images.unsplash.com/photo-1568376794508-ae52c6ab3929?q=80&w=2000&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    description:
        "Sweet, savory, and sensational. A dessert-first dining experience.",
    name: "Sushi Spot",
  ),
];
